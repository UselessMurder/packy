...
trap::trap() : tag_container() {
  id = global::cs.generate_unique_number("lolo");
}
trap::trap(std::function<void(eg::key_value_storage &values,
...
void trap::set_body(std::function<void(eg::key_value_storage &values,
                                       global::flag_container flags)>
                        current_body) {
  body = current_body;
}

void trap::execute(eg::key_value_storage &values,
                   global::flag_container flags) {
  body(values, flags);
}
...
void base_mk::add_trap(std::string trap_name,
                       std::function<void(eg::key_value_storage &values,
                                          global::flag_container flags)>
                           trap_code) {
  traps[trap_name].set_body(trap_code);
}
...
void base_mk::insert_trap(std::string name, uint64_t id,
                          global::flag_container flags) {
...
void base_mk::insert_random_trap(
    std::initializer_list<std::string> current_tags, uint64_t id,
    global::flag_container flags) {
  std::vector<trap *> tmp;
  for (auto t : traps) {
    if (t.second.check_tags(current_tags)) tmp.push_back(&(traps[t.first]));
  }
